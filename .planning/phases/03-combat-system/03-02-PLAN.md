...
<task type="auto">
  ...
  <verify>
    ...
    Verify that Prediction.ts sendInput includes the `fire` field in the input message sent to the server (grep for `fire` in the sendInput method).
    ...
    - Pressing spacebar fires projectiles (confirming fire propagates through Prediction.ts to server)
