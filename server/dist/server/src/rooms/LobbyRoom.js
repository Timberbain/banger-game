...
const MatchmakingQueue_1 = require("./MatchmakingQueue");
/**
...
class LobbyRoom extends colyseus_1.Room {
    constructor() {
        super(...arguments);
        this.maxClients = lobby_1.LOBBY_CONFIG.MAX_PLAYERS;
    }
    onCreate(options) {
        this.setState(new LobbyState_1.LobbyState());
        // Setup private room if requested
        if (options.private) {
            this.state.roomCode = (0, roomCode_1.generateRoomCode)(lobby_1.LOBBY_CONFIG.ROOM_CODE_LENGTH);
        ...
        this.onMessage("selectRole", (client, message) => {
            ...
            const { role } = message;
            // Validate role
            if (!lobby_1.VALID_ROLES.includes(role)) {
        ...
        this.onMessage("joinQueue", (client, message) => {
            ...
            if (!lobby_1.VALID_ROLES.includes(preferredRole)) {
                return;
            }
            MatchmakingQueue_1.matchmakingQueue.addToQueue(client.sessionId, preferredRole);
        });
        this.onMessage("leaveQueue", (client, message) => {
            MatchmakingQueue_1.matchmakingQueue.removeFromQueue(client.sessionId);
        });
        // Periodic matchmaking check (every 1 second)
        this.matchmakingCheckInterval = this.clock.setInterval(() => {
            const match = MatchmakingQueue_1.matchmakingQueue.tryFormMatch();
            if (match) {
            ...
            // Check for timeouts
            const timedOut = MatchmakingQueue_1.matchmakingQueue.checkTimeouts(lobby_1.LOBBY_CONFIG.QUEUE_TIMEOUT);
    ...
    onLeave(client, consented) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (consented) {
                ...
                try {
                    yield this.allowReconnection(client, lobby_1.LOBBY_CONFIG.LOBBY_RECONNECT_GRACE);
    ...
    checkReadyToStart() {
        ...
        if (players.length !== lobby_1.LOBBY_CONFIG.MAX_PLAYERS) {
            return;
        }
        ...
        const validDistribution = roleCounts.paran === 1 &&
            roleCounts.faran === 1 &&
            roleCounts.baran === 1;
    ...
    startCountdown() {
        ...
        this.state.countdown = lobby_1.LOBBY_CONFIG.COUNTDOWN_SECONDS;
    ...
    startMatch() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const room = yield colyseus_1.matchMaker.createRoom("game_room", {
                    fromLobby: true,
                    roleAssignments,
                });
